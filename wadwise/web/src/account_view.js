...
function AccountStatus({ account, balance, cur_list }) {
    ...
    if (account.is_sheet) {
        return separateOnMultiCurs(
            cur_list.full,
            totalsRows(cur_list.total, balance.current_total, 'Balance' + (open.value ? '' : ' [+]'), {
...
function TransactionList({ account, transactions, amap, urls }) {
    ...
    return h(
        'v-stack.gap-xl',
        head.map(([gdt, tlist]) => h('div', h('div.transaction-date', gdt), h('v-stack', tlist.map(Transaction)))),
        showTail.value &&
            tail.map(([gdt, tlist]) => h('div', h('div.transaction-date', gdt), h('v-stack', tlist.map(Transaction)))),
    )
}
